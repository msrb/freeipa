...
class SSHPublicKey(object):
    ...
    def __init__(self, key, comment=None, options=None, encoding='utf-8'):
        ...
        # We allow:
        # - raw blob, as bytes
        # - base64-encoded key, as str
        # - openssh-formatted key, as unicode
        # In Python 2, bytes (str) are decoded to unicode, if not a raw blob.

        if six.PY2:
            # All valid public key blobs start with 3 null bytes (see RFC 4253
            # section 6.6, RFC 4251 section 5 and RFC 4250 section 4.6)
            if isinstance(key, bytes) and key[:3] != '\0\0\0':
                key = key.decode(encoding)

        if isinstance(key, bytes):
            valid = self._parse_raw(key)
        else:
            valid = self._parse_base64(key) or self._parse_openssh(key)
